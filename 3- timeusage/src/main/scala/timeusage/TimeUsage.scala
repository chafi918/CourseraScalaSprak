...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val headC = StructField(columnNames.head, StringType, false)
    val allRest = columnNames.tail.map(node => StructField(node, DoubleType, false))
    StructType(headC :: allRest)
  }
  ...
  def row(line: List[String]): Row = {
    val headV = line.head
    val restV = line.tail.map(_.toDouble)

    Row.fromSeq(headV :: restV)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    (
      ...
      columnNames.filter(!_.matches("^(t1801|t1803|t1805).*")).filter(_.matches("^(t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18).*")).map(new Column(_))
    )
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs".isin(1, 2), "working").otherwise("not working").as("working")
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(row => TimeUsageRow(row.getAs[String]("working"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age))
      ...
      .map(data => TimeUsageRow(data._1._1, data._1._2, data._1._3, Math.round(data._2 * 10) / 10d, Math.round(data._3 * 10) / 10d, Math.round(data._4 * 10) / 10d))
      .orderBy("working", "sex", "age")
  }
}

...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.textFile(WikipediaData.filePath).map( wD => WikipediaData.parse(wD))
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.filter(_.text.split(" ").contains(lang)).collect().length
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(wA => langs.filter(lang => wA.text.split(" ").contains(lang))
      .map(lang => (lang, wA)))
      .groupByKey()
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(i => (i._1, i._2.size)).collect().toList.sortBy(_._2).reverse
  }
